...
int parse_opts(int argc, char **argv, int *ifindex, struct ndisc_pkt *pkt)
{
	...
	if (argc != 9) {
		fprintf(stderr, "Usage: %s <iface> <dst_mac> <src_mac> <dst_ip> <src_ip> <target_ip> <op> <lladr>\n",
			argv[0]);
		return -1;
	}
	...
	*ifindex = atoi(argv[1]);
	...
	if (!mac) {
		fprintf(stderr, "Unable to parse dst_mac from '%s'\n", argv[2]);
		return -1;
	}
	...
	if (!mac) {
		fprintf(stderr, "Unable to parse src_mac from '%s'\n", argv[3]);
		return -1;
	}
	...
	if (inet_pton(AF_INET6, argv[4], &pkt->ip6.daddr) != 1) {
		fprintf(stderr, "Unable to parse dst_ip from '%s'\n", argv[4]);
		return -1;
	}
